...
function DriveFileItemComponent({ item, onPress, onOptionsPress }: DriveFileItemProps) {
  ...
  if (item.isFolder) {
    return (
      <View className="flex-row items-center bg-[#F8F9FA] border border-[#E8EAED] rounded-2xl h-[72px]">
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={handleRowPress}
          className="flex-1 flex-row items-center p-3.5 pr-0 h-full"
        >
  ...
  return (
    <View className="flex-row items-center bg-white border border-[#EBECEF] rounded-2xl h-[72px]">
      <TouchableOpacity
        activeOpacity={0.7}
        onPress={handleRowPress}
        className="flex-1 flex-row items-center p-3.5 pr-0 h-full"
